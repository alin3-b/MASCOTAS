<?php
require 'db.php';  // Este archivo debe contener la conexión PDO como $pdo
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nombre === '' || $email === '' || $codigoPostal === '') {
        ...
        // Verificar que no haya otro cliente con el mismo email
        $stmtCheck = $pdo->prepare("SELECT id FROM clientes WHERE email = ? AND id != ?");
        ...
        if ($existe) {
            ...
            // Actualizar cliente
            $stmtUpdate = $pdo->prepare("UPDATE clientes SET nombre = ?, email = ?, codigo_postal = ? WHERE id = ?");
            $stmtUpdate->execute([$nombre, $email, (int)$codigoPostal, $id]);
