...
<body>
  ...
  <main class="container mt-4">
    ...
    <?php
    try {
        // Convertimos el cursor en array para evitar errores de iteración
        $datos = iterator_to_array($mascotas->find([], ['sort' => ['id' => 1]]));
    } catch (Exception $e) {
      ...
      <table class="table table-bordered table-striped">
        ...
        <tbody>
          <?php foreach ($datos as $m): 
            $fechaNac = isset($m['fecha_nac']) && method_exists($m['fecha_nac'], 'toDateTime')
                ? $m['fecha_nac']->toDateTime()->format('Y-m-d')
                : 'N/A';
            $vendido = isset($m['vendido']) && $m['vendido'] ? 'Sí' : 'No';
